                    ...
                    this.jsun.getJSON(data[i].git_url).subscribe(data => {
                        ...
                        for (let j = 0; j < mainData.length; j++) {
                            if (mainData[j].codename == this.getRouteDevice(this.route.url)) {
                                ...
                                const sfUrl = 'https://sourceforge.net/projects/superioros/files/' + mainData[j].codename;
                                this.sf.getBuilds(sfUrl).subscribe(data => {
                                    console.log(data);
                                });
                            }
                        }
...
let SourceforgeService = class SourceforgeService {
    ...
    getBuilds(url) {
        return this.http.get(url, { headers: {
            ...
            let builds = [];
            let parsedHtml = this.parser.parseFromString(data.toString(), 'text/html');
            let beeld = parsedHtml.querySelectorAll("#files_list > tbody > tr");
            for (let i = 0; i < beeld.length; i++) {
                builds.push({
                    title: beeld[i].title,
